"use client";

import { useCallback, useEffect, useRef, useState } from "react";
import { ChevronLeft, ChevronRight } from "lucide-react";
import { adsApi, type AdSlidePublic } from "@/lib/api";
import { useAuthStore } from "@/store/auth";
import { useTranslation } from "@/hooks/useTranslation";

const ROTATE_MS = 5500;

export function AdCarousel() {
  const { t } = useTranslation();
  const lowBandwidth = useAuthStore((s) => s.lowBandwidth);
  const [slides, setSlides] = useState<AdSlidePublic[]>([]);
  const [index, setIndex] = useState(0);
  const [paused, setPaused] = useState(false);
  const touchX = useRef<number | null>(null);

  useEffect(() => {
    if (lowBandwidth) return;
    let cancelled = false;
    adsApi
      .active()
      .then(({ data }) => {
        if (!cancelled && Array.isArray(data)) {
          setSlides(data.filter((s) => Boolean(s.image_url)));
          setIndex(0);
        }
      })
      .catch(() => {
        if (!cancelled) setSlides([]);
      });
    return () => {
      cancelled = true;
    };
  }, [lowBandwidth]);

  const count = slides.length;
  const go = useCallback(
    (dir: -1 | 1) => {
      if (count < 2) return;
      setIndex((i) => (i + dir + count) % count);
    },
    [count]
  );

  useEffect(() => {
    if (lowBandwidth || paused || count < 2) return;
    const id = window.setInterval(() => go(1), ROTATE_MS);
    return () => window.clearInterval(id);
  }, [lowBandwidth, paused, count, go]);

  if (lowBandwidth || count === 0) return null;

  const slide = slides[index];
  if (!slide) return null;

  const content = (
    // eslint-disable-next-line @next/next/no-img-element
    <img
      src={slide.image_url}
      alt={slide.title}
      className="w-full h-full object-cover"
      draggable={false}
    />
  );

  return (
    <aside
      className="ad-carousel border-t border-[var(--border-subtle)] bg-black/30"
      aria-label={t("ads.carouselLabel")}
      onMouseEnter={() => setPaused(true)}
      onMouseLeave={() => setPaused(false)}
      onTouchStart={(e) => {
        touchX.current = e.changedTouches[0]?.clientX ?? null;
      }}
      onTouchEnd={(e) => {
        const start = touchX.current;
        const end = e.changedTouches[0]?.clientX;
        touchX.current = null;
        if (start == null || end == null) return;
        const delta = end - start;
        if (Math.abs(delta) < 40) return;
        go(delta < 0 ? 1 : -1);
      }}
    >
      <div className="relative max-w-7xl mx-auto">
        <div className="relative w-full aspect-[6/1] min-h-[72px] max-h-[140px] overflow-hidden">
          {slide.link_url ? (
            <a
              href={slide.link_url}
              target="_blank"
              rel="noopener noreferrer sponsored"
              className="block w-full h-full"
              aria-label={slide.title}
            >
              {content}
            </a>
          ) : (
            <div className="w-full h-full">{content}</div>
          )}
        </div>

        {count > 1 && (
          <>
            <button
              type="button"
              onClick={() => go(-1)}
              className="absolute left-2 top-1/2 -translate-y-1/2 p-1.5 rounded-full bg-black/45 text-white/90 hover:bg-black/65"
              aria-label={t("ads.prev")}
            >
              <ChevronLeft size={18} />
            </button>
            <button
              type="button"
              onClick={() => go(1)}
              className="absolute right-2 top-1/2 -translate-y-1/2 p-1.5 rounded-full bg-black/45 text-white/90 hover:bg-black/65"
              aria-label={t("ads.next")}
            >
              <ChevronRight size={18} />
            </button>
            <div className="absolute bottom-2 left-1/2 -translate-x-1/2 flex gap-1.5">
              {slides.map((s, i) => (
                <button
                  key={s.id}
                  type="button"
                  aria-label={`${i + 1} / ${count}`}
                  aria-current={i === index}
                  onClick={() => setIndex(i)}
                  className={
                    i === index
                      ? "w-2 h-2 rounded-full bg-africhess-gold"
                      : "w-2 h-2 rounded-full bg-white/40 hover:bg-white/70"
                  }
                />
              ))}
            </div>
          </>
        )}
      </div>
      <p className="sr-only" aria-live="polite">
        {slide.title}
      </p>
    </aside>
  );
}
